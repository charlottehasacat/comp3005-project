...
SELECT
    ...
    AVG((e.event_details->'statsbomb_xg')::numeric) AS average_xg_score
FROM
    competitions c
    JOIN matches m
        ON c.competition_id = m.competition_id AND
...
SELECT
    p.name AS player_name,
    COUNT(*) AS number_of_first_time_shots
FROM
    competitions c
    JOIN matches m
        ON c.competition_id = m.competition_id AND
...
WHERE
    ...
    e.event_type = 'shot' AND
    (event_details->'first_time')::boolean = TRUE
GROUP BY p.player_id
ORDER BY
...
SELECT
    p.name AS player_name,
    COUNT(*) AS times_being_recipient_of_passes
FROM
    competitions c
    JOIN matches m
        ON c.competition_id = m.competition_id AND
    ...
    JOIN events e
        ON m.match_id = e.match_id
    JOIN players p
        ON (e.event_details->'recipient'->'id')::integer = p.player_id
WHERE
    c.competition_name = 'Premier League' AND
    c.season_name = '2003/2004' AND
    e.event_type = 'pass' AND
    e.event_details->'recipient' IS NOT NULL
GROUP BY p.player_id
ORDER BY
    times_being_recipient_of_passes DESC,
    player_name ASC;
"""
...
SELECT
    p.name AS player_name,
    COUNT(*) AS number_of_through_balls
FROM
    competitions c
    JOIN matches m
        ON c.competition_id = m.competition_id AND
...
WHERE
    ...
    c.season_name = '2020/2021' AND
    e.event_type = 'pass' AND
    e.event_details->'technique' IS NOT NULL AND
    e.event_details->'technique'->>'name' = 'Through Ball'
GROUP BY p.player_id
ORDER BY
...
WHERE
    c.competition_name = 'La Liga' AND
    c.season_name IN ('2020/2021', '2019/2020', '2018/2019') AND
    e.event_type = 'dribble' AND
    e.event_details->'outcome' IS NOT NULL AND
    e.event_details->'outcome'->>'name' = 'Complete'
GROUP BY p.player_id
ORDER BY
...
WHERE
    c.competition_name = 'La Liga' AND
    c.season_name = '2020/2021' AND
    e.event_type = 'dribbled past'
GROUP BY p.player_id
ORDER BY
    times_dribbled_past DESC,
    player_name ASC;
"""
